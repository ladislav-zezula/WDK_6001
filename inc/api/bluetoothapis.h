...
// ***************************************************************************
//
//  EIR Record Enumeration
//
// ***************************************************************************

typedef struct _BLUETOOTH_EIR_SEARCH_PARAMS {

    DWORD   dwSize;                         //  IN sizeof this structure.
    
    const BLUETOOTH_DEVICE_INFO * pbtdi;    //  IN pointer to the device info structure of the remote device.
   
    HANDLE  hRadio;                         //  IN the radio to search on. NULL == all radios

} BLUETOOTH_EIR_SEARCH_PARAMS;

typedef HANDLE      HBLUETOOTH_EIR_RECORD_FIND;

typedef struct _BLUETOOTH_EIR_RECORD_INFO {

    DWORD   dwSize;                         // IN sizeof this structure

    DWORD   cbRecordSize;                  // OUT size of EIR data (including type). 

    LPBYTE  pbRecord;                      // OUT pointer to EIR data type and data.

} BLUETOOTH_EIR_RECORD_INFO;

//
//  Description:
//      Begins the enumeration of Extended Inquiry Response (EIR) records.
//
//  Parameters:
//      pbtspIn
//          A pointer to a BLUETOOTH_EIR_SEARCH_PARAMS structure. This
//          structure contains the flags and inputs used to conduct the search.
//          See BLUETOOTH_EIR_SEARCH_PARAMS for details.
//
//      pEirRecordInfoInout
//          A pointer to a BLUETOOTH_EIR_RECORD_INFO structure to return information 
//          about the first EIR record for this device. Note that the dwSize 
//          member of the structure must be the sizeof(BLUETOOTH_EIR_RECORD_INFO) 
//          before calling because the APIs hast to know the size of the buffer 
//          being past in. The dwSize member must also match the exact 
//          sizeof(BLUETOOTH_EIR_RECORD_INFO) or the call will fail.
//
//  Return Values:
//      NULL
//          Error opening radios, not records found or EIR data not supported. 
//          Use GetLastError for more info.
//
//          ERROR_INVALID_PARAMETER
//              The pbtspIn or pEirRecordInfoInout parameter is NULL.
//
//          ERROR_INVALID_HANDLE
//              The radio handle is invalid.
//
//          ERROR_REVISION_MISMATCH
//              The pbtspIn or pEirRecordInfoInout structure is not the right length.
//
//          ERROR_RESOURCE_DATA_NOT_FOUND
//              The device does not support EIR records. 
//
//          ERROR_NOT_SUPPORTED
//              The local system does not support this feature. 
//
//          ERROR_INVALID_DATA
//              Invalid EIR records.
//
//          ERROR_NO_MORE_ITEMS
//              No more EIR records found.
//
//          other Win32 errors
//
//      any other value
//          Success. The return handle is valid and pEirRecordInfoInout points to 
//          valid data.
//
__checkReturn
HBLUETOOTH_EIR_RECORD_FIND
WINAPI
BluetoothFindFirstEirRecord(
      __in const BLUETOOTH_EIR_SEARCH_PARAMS * pbtspIn
    , __inout BLUETOOTH_EIR_RECORD_INFO * pEirRecordInfoInout
    );

//
//  Description:
//      Finds the next EIR record in the enumeration.
//
//  Parameters:
//      hFindIn
//          The handle returned from BluetoothFindFirstEirRecord().
//
//      pEirDataInfoInout
//          A pointer to a BLUETOOTH_EIR_RECORD_INFO structure to return information
//          about the next EIR record found. Note that the dwSize member of the
//          structure must be the sizeof(BLUETOOTH_EIR_RECORD_INFO) before
//          calling because the APIs hast to know the size of the buffer being
//          past in. The dwSize member must also match the exact 
//          sizeof(BLUETOOTH_EIR_RECORD_INFO) or the call will fail.
//
//  Return Values:
//      TRUE
//          Next EIR record found. pEirRecordInfoInout points to valid info.
//
//      FALSE
//          No record found. Call GetLastError() for more details.
//
//          ERROR_INVALID_HANDLE
//              hFindIn is NULL or invalid.
//
//          ERROR_INVALID_PARAMETER
//              The pEirRecordInfoInout parameter is NULL.
//
//          ERROR_REVISION_MISMATCH
//              The structure pointed to by pEirRecordInfoInout is not the right 
//              size or the structures dwSize member was not set prior to calling.
//
//          ERROR_INVALID_DATA
//              Invalid EIR records.
//
...
BOOL
WINAPI
BluetoothFindNextEirRecord(
      __in HBLUETOOTH_EIR_RECORD_FIND hFindIn
    , __inout BLUETOOTH_EIR_RECORD_INFO * pEirRecordInfoInout
    );
...
BOOL
WINAPI
BluetoothFindEirRecordClose (
      __in HBLUETOOTH_EIR_RECORD_FIND hFindIn
    );

// ***************************************************************************
//
//  EIR GUID Enumeration
//
// ***************************************************************************

typedef HANDLE      HBLUETOOTH_EIR_GUID_FIND;

//
//  Description:
//      Begins the enumeration of Extended Inquiry Response (EIR) GUIDs.
//
//  Parameters:
//      pbtspIn
//          A pointer to a BLUETOOTH_EIR_SEARCH_PARAMS structure. This
...
//
//      pGuidOut
//          A pointer to a GUID element to return information about the first 
//          EIR GUID for this device. 
//
//  Return Values:
//      NULL
//          Error opening radios, not GUIDs found or EIR data not supported. 
//          Use GetLastError for more info.
//
//          ERROR_INVALID_PARAMETER
//              The pbtspIn or pGuidOut parameter is NULL.
//
//          ERROR_INVALID_HANDLE
//              The radio handle is invalid.
//
//          ERROR_REVISION_MISMATCH
//              The pbtspIn structure is not the right length.
//
//          ERROR_RESOURCE_DATA_NOT_FOUND
//              The device does not support EIR records. 
//
...
//
//          ERROR_NO_MORE_ITEMS
//              No more EIR GUIDs. EIR contains the full list of SDP service guids.
//
//          ERROR_MORE_DATA
//              No more EIR GUIDs. EIR contains only a partial list of SDP service guids.
//              Use SDP search for retrieving complete list.
//
//          other Win32 errors
//
//      any other value
//          Success. The return handle is valid and pGuidOut points to a 
//          valid GUID.
//
__checkReturn
HBLUETOOTH_EIR_GUID_FIND
WINAPI
BluetoothFindFirstEirGuid (
      __in const BLUETOOTH_EIR_SEARCH_PARAMS * pbtspIn
    , __out GUID * pGuidOut
    );
...
//
//  Description:
//      Finds the next EIR GUID in the enumeration.
//
//  Parameters:
//      hFindIn
//          The handle returned from BluetoothFindFirstEirGuid().
//
//      pGuidOut
//          A pointer to a GUID structure to return information about the next 
//          EIR GUID found.
//
//  Return Values:
//      TRUE
//          Next EIR GUID found. pGuidOut points to valid info.
//
//      FALSE
//          No GUID found. Call GetLastError() for more details.
//
//          ERROR_INVALID_HANDLE
//              hFindIn is NULL or invalid.
//
//          ERROR_INVALID_PARAMETER
//              The pGuidOut parameter is NULL.
//
//          ERROR_INVALID_DATA
//              Invalid EIR data.
//
...
BOOL
WINAPI
BluetoothFindNextEirGuid(
      __in HBLUETOOTH_EIR_GUID_FIND hFindIn
    , __out GUID * pGuidOut
    );
...
//
//          ERROR_INVALID_HANDLE
//              The handle is NULL or invalid.
//
BOOL
WINAPI
BluetoothFindEirGuidClose (
      __in HBLUETOOTH_EIR_GUID_FIND hFindIn
    );
...
// ***************************************************************************
//
//  EIR Record Publishing
//
// ***************************************************************************

typedef HANDLE   HBLUETOOTH_EIR_RECORD;

//
//  Description:
//      Publishes a Extended Inquiry Response (EIR) record.
//
//  Parameters
//      hRadioIn
...
//
//      pbRecordIn
//          Pointer to a EIR record (includes: data type + data).
//
//      cbRecordSizeIn
//          The size of EIR record above in bytes.
//
//  Return Values:
//      NULL
//          Error opening radios, EIR data not supported, out of resources, etc. 
//          Use GetLastError for more info.
//
//          ERROR_INVALID_HANDLE
//              The radio handle is invalid.
//
//          ERROR_INVALID_PARAMETER
//              The pbRecordIn is NULL or cbRecordSizeIn is zero.
//
//          ERROR_RESOURCE_DATA_NOT_FOUND
//              The device does not support EIR records. 
//
...
//
//          ERROR_INVALID_DATA
//              The EIR record is invalid.
//
//          other Win32 errors
//
//      any other value
//          Success. The return EIR record handle is valid.
//
__checkReturn
HBLUETOOTH_EIR_RECORD 
WINAPI
BluetoothSetEirRecord (
      __in_opt HANDLE  hRadioIn
    , __in_bcount(cbRecordSizeIn) BYTE * pbRecordIn
    , __in DWORD cbRecordSizeIn
    );

//
//  Description:
//      Updates a published Extended Inquiry Response (EIR) record.
//
//  Parameters
//      hRecordIn
//          Handle of a EIR record to update.
//
//      pbRecordIn
//          Pointer to a EIR record (includes: data type + data).
//
...
BOOL
WINAPI
BluetoothUpdateEirRecord (
      __in HBLUETOOTH_EIR_RECORD hRecordIn
    , __in_bcount(cbRecordSizeIn) BYTE * pbRecordIn
    , __in DWORD cbRecordSizeIn
    );
...
//
//  Description:
//      Closes a EIR record handle.
//
//  Parameters:
//      hRecordIn
//          The handle returned from BluetoothSetEirRecord().
//
//  Return Values:
//      TRUE
//          Handle succesfully closed.
...
BluetoothRegisterForAuthenticationEx(
      __in const BLUETOOTH_DEVICE_INFO * pbtdiIn
