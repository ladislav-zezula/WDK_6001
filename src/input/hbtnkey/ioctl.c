    ...
    else
    {
        ...
        if (Irp->Cancel)
        {
            ...
            if (OldCancelRoutine != NULL)
            {
                ...
                //
                TAssert(OldCancelRoutine == ReadReportCanceled);
    ...
    )
{
    ...
    TEnter(Func,("(DevObj=%p,Irp=%p)\n", DevObj, Irp));

    TAssert(Irp->Cancel);
    TAssert(Irp->CancelRoutine == NULL);
    ...
    IoReleaseRemoveLock(&devext->RemoveLock, Irp);
    IoCompleteRequest(Irp, IO_NO_INCREMENT);
    TInfo(("ReadReport IRP was canceled.\n"));
