      ...
      //1 IRB_FUNCTION_LUN_RESET => AhciHwReset
        case (IRB_FUNCTION_LUN_RESET):
            if (AhciHwReset (ChannelExtension)) {
                ...
                ChannelExtension->Reserved0 |= (1 << 7);
                AhciCompleteRequest(ChannelExtension, Irb);
...
--*/
{
  ...
  //1.1 Keep track of the completion for our debugging records
    ...
    AtaPortMoveMemory(&cmdHistory->CompletionFIS, &ChannelExtension->ReceivedFIS->D2hRegisterFis, 20);
    AtaPortMoveMemory(&cmdHistory->CompletionPx, ChannelExtension->Px, 0x40);
...
--*/
{
  ...
  //1.2 Perform the IRB's last request, if there was one
    if (Irb->Reserved) {
        completionRoutine = Irb->Reserved;
        completionRoutine(ChannelExtension);
        Irb->Reserved = NULL;
    }
...
--*/
{
  ...
  //2.1 Copy over the Channel Extension header
    ...
    if(ChannelExtension->ControllerIS) {
        AtaPortMoveMemory(&ChannelExtension->ExecutionHistory[ChannelExtension->ExecutionHistoryNextAvailableIndex].IS, &ChannelExtension->StateFlags,sizeof(USHORT));
    } else {
        ChannelExtension->ExecutionHistory[ChannelExtension->ExecutionHistoryNextAvailableIndex].IS = ~0;
    }
    AtaPortMoveMemory(&ChannelExtension->ExecutionHistory[ChannelExtension->ExecutionHistoryNextAvailableIndex].ChannelExtension,ChannelExtension,24);
  //2.2 Copy over the AHCI registers
    if(ChannelExtension->Px) {
        AtaPortMoveMemory(&ChannelExtension->ExecutionHistory[ChannelExtension->ExecutionHistoryNextAvailableIndex].Px, ChannelExtension->Px,0x40);
    } else {
        for(i=0; i<0x10; i++) {
            ChannelExtension->ExecutionHistory[ChannelExtension->ExecutionHistoryNextAvailableIndex].Px[i] = ~0;
        }
    }
}
...
--*/
{
  ...
  //1 Fills in the local IRB with the SetFeatures command
    ...
    irb->IrbExtension = ChannelExtension->Local.IrbExtension;
    irb->DataBuffer = NULL;               
