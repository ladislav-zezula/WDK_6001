...
*/
    RecordExecutionHistory(ChannelExtension, 0x00000012);//AhciPortReset AhciHwReset wrapper
...
*/
  ...
  //2.1 Complete the command being issued if there was one
    ...
    //2.2 Restore the unsent programmed commands for careful reprocessing
    channelExtension->SlotManager.SingleIoSlice |= ci;  //put the commands that didn't get a chance to finish back into the single queue for carefull processing
...
*/
{
  ...
  //3.1 Clear SERR
    AtaPortWriteRegisterUlong(&Px->SERR.AsUlong, ~0);
    ...
    if (ssts.DET == 0)  { //When a COMRESET is sent to the device the PxSSTS.DET field shall be cleared to 0h.
      ...
      //1.2 Wait for 1 second for DET to show signs of life
            ...
            if (ChannelExtension->StateFlags.Crashing) {
                ...
                AtaPortRequestTimer(ChannelExtension, P_Running, 10000); //10 milliseconds
            }
            return;
        }
    ...
    if ( ( tfd.STS.BSY == 0) && ( tfd.STS.DRQ == 0) ) {
      ...
      //2.1 Wait for the rest of the 4 seconds for BSY and DRQ to clear      
            ...
            if (ChannelExtension->StateFlags.Crashing) {
                ...
                AtaPortRequestTimer(ChannelExtension, P_Running, 20000); //20 milliseconds
            }
            return;
        }
    }
