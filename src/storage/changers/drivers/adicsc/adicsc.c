                ...
                if (elementType == ChangerDrive) {
                    if (outputBuffLen >=
                        ...
                        if (idLength != 0) {
                            ...
                            // ADIC SCALAR 100 Drive serial number is at offset
                            // VENDOR_ID_LEN+PRODUCT_ID_LEN=24.

                            if(changerData->DriveID == ADIC_SCALAR_100)	 {
                                RtlMoveMemory(elementStatusEx->SerialNumber,
                                   (idField+VENDOR_ID_LENGTH+PRODUCT_ID_LENGTH),
                                    idLength);
                            } else {
                                RtlMoveMemory(elementStatusEx->SerialNumber,
                                              idField,
...

{
    ...
    switch (asc) {
        ...
        case 0x83:
            if ((asq == 0x03) ||
                (asq == 0x00 && 
                ((DriveID == ADIC_SCALAR) || (DriveID == ADIC_SCALAR_100)) ) ) {
