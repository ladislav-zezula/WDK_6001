    ...
    if (SRB_STATUS(srb.SrbStatus) == SRB_STATUS_SUCCESS ||
        ...
        if (RtlCompareMemory(changerData->InquiryData.VendorId,"HP      ",8) == 8) {
            if (RtlCompareMemory(changerData->InquiryData.ProductId,"C1553A",6) == 6) {
                ...
                changerData->DriveID = HP_DAT72X6;
            }
...

{
    ...
    if (changerData->DriveID == HP_DAT72X6) {
...
ULONG CheckHpMagazineAccessible(
    IN PDEVICE_OBJECT DeviceObject,
    IN PIRP Irp
    )

{
    ...
    NTSTATUS status;
    ULONG    length;
    ULONG    hp_magazine_accessible = 1;

    length = sizeof(HP_ELEMENT_DESCRIPTOR);
    status = RtlULongAdd(sizeof(ELEMENT_STATUS_HEADER), length, &length);
    if (!NT_SUCCESS(status)) {
        return hp_magazine_accessible;  
    }
    status = RtlULongAdd(sizeof(ELEMENT_STATUS_PAGE), length, &length);
    if (!NT_SUCCESS(status)) {
        return hp_magazine_accessible;  
    }
    ...
    cdb->READ_ELEMENT_STATUS.ElementType = (UCHAR)ChangerDrive;
    ...
    cdb->READ_ELEMENT_STATUS.StartingElementAddress[0] = (UCHAR)0;
    cdb->READ_ELEMENT_STATUS.StartingElementAddress[1] = (UCHAR)1;

    cdb->READ_ELEMENT_STATUS.NumberOfElements[0] = (UCHAR)(0);
    cdb->READ_ELEMENT_STATUS.NumberOfElements[1] = (UCHAR)(1);
    ...
    if (NT_SUCCESS(status)) {
        ...
        //
        // The buffer is composed of a header, status page, and element descriptor.
        ...
        if (!(elementDescriptor->SValid)) hp_magazine_accessible = 0;
    } 
...

{
    ...
    if((changerData->DriveID == HP_DAT72X6) && 
       (originalElementType == ChangerSlot)) {

       hp_magazine_accessible = CheckHpMagazineAccessible(DeviceObject, Irp);
    }
    ...
    if ((NT_SUCCESS(status)) ||
        ...
        // In HPDAT72x6 changers the accessible state of magazine
        // can be determined by checking the SValid bit in Data
        // Transfer Element descriptor. If SValid is set then 
        // magazine Accessible.

        if((changerData->DriveID == HP_DAT72X6) && 
           (originalElementType == AllElements) &&
           (elementType == ChangerDrive)        &&
           !(elementDescriptor->SValid)) {

            hp_magazine_accessible = 0;
         } 
