...
Routine Description
    ...
    Target - Handle to the Iotarget to which teh Request was sent. COnceptually this
              is the BULK USB OUT pipe(on of  Data or beacon) 
...

{
    ...
    if (!NT_SUCCESS(status)) {
        ...
        if (usbCompletionParams->UsbdStatus == USBD_STATUS_DEV_NOT_RESPONDING) {
            WdfUsbTargetDeviceResetPortSynchronously(writeContext->Nic->UsbDevice);
        }
        if (nicTxd != NULL) {
            //
