    ...
    )
{
    ...
    if (pCard->NumChannels == 0)
    {
        ...
        for (PortIndex = 0; PortIndex < pCard->NumPorts; PortIndex++)
        {
            ...
            ASSERT(pCard->pPortArray[PortIndex]->NumChannels + pCard->NumChannels > pCard->NumChannels);
            if (pCard->pPortArray[PortIndex]->NumChannels + pCard->NumChannels < pCard->NumChannels)
            {           
                DBG_ERROR(DbgInfo,("Number of Channels exceeded MAX_ULONG: %d + %d\n",
