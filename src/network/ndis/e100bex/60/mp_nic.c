            ...
            NetBufferToSend = NET_BUFFER_NEXT_NB(NetBufferToSend))
    {
...

        {
            ...
            if (NDIS_STATUS_SUCCESS != SendStatus)
            {
                ...
                // Revert the State
                pMpTcb->HwTcb->TxCbHeader.CbStatus = CbStatusState;
...
--*/
{
    ...
    // Update the CbStatus in MiniportSendNetBufferList
    // not here pHwTcb->TxCbHeader.CbStatus = 0;
