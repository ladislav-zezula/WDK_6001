    ...
    *AddressingReset = TRUE;
    ...
    do
    {
        ...
        if (NDIS_CURRENT_IRQL() == DISPATCH_LEVEL)
        {
            ...
            // Schedule the Reset Work Item, we want to be sure
            // that we can safely terminate long operation when 
            // we are at DISPATCH
            NdisQueueIoWorkItem(
                Adapter->NdisResetWorkItem,
                MpResetWorkItem,
                Adapter
                );
                
            DBGPRINT_S(Status, ("<==== MPReset, Status=%x\n - Reset Work Item Scheduled.", Status));
        }
        else
        {
            Status = MPE100Reset(Adapter);
            DBGPRINT_S(Status, ("<==== MPReset, Status=%x\n", Status));
        }
    } 
    while (FALSE);
