...
--*/    
{
    ...
    *pAdapter = NULL;
    ...
    do
    {
        ...
        Adapter->NdisResetWorkItem = NdisAllocateIoWorkItem(MiniportAdapterHandle);
        if(Adapter->NdisResetWorkItem == NULL)
        {
            Status = NDIS_STATUS_RESOURCES;
            DBGPRINT(MP_ERROR, ("Failed to allocate Ndis Reset Workitem \n"));        
            break;
        }
...
--*/    
{
    ...
    // Free the work item
    if(Adapter->NdisResetWorkItem)
    {
        NdisFreeIoWorkItem(Adapter->NdisResetWorkItem);
    }
...
--*/    
{
    ...
    ULONGLONG       TcbLength, SendBufLength;
    ...
    if (!NT_SUCCESS(RtlULongMult(Adapter->NumTcb,
                                 ...
                                 &Adapter->NumTbd)))
    {      
            DBGPRINT(MP_ERROR, ("Integer overflow while setting NumTbd in Adapter structure\n"));
...
--*/    
{
    ...
    do
    {
        ...
        for (RfdCount = Adapter->NumRfd; RfdCount > NIC_MIN_RFDS; RfdCount >>= 1)
        {
            ...
            // Integer overflow check
            //
            if (FAILED(ULongMult(Adapter->HwRfdSize, RfdCount, &ulLength)))
            {
                Adapter->HwRfds = NULL;
                break;
            }
