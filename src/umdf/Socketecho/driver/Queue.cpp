...
--*/
{
    ...
    HANDLE SocketHandle = NULL; 
     ...
     CComObject<CMyDevice> * myDevice = NULL;
    ...
    hr = pDevice->RetrieveContext((void**)&myDevice);
    ...
    myDevice->GetDeviceConnectionParams(&hostStr,&portStr);
    
    //
    // Connect to the socket server using device connection params
    //
    
    hr = pConnection->Connect(hostStr,portStr);
    ...
    //
    SocketHandle = pConnection->GetSocketHandle();    
    ...
    FileContext *pContext  = new  FileContext; 
...
--*/   
{
    ...
    pWdfRequest->SetCompletionCallback(
                                    CComQIPtr<IRequestCallbackRequestCompletion>(this),
                                    NULL);
    ...
    //
   // Send the request to the win32 i/o target . 
   // This target was created in OnCreateFile
   //

    hr = pWdfRequest->Send(
                                        pContext->pFileTarget,
                                        0,
                                        0);
Exit:
