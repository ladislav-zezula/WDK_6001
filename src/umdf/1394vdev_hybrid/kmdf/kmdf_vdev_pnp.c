...
--*/
{
    ...
    //
    // Protect against just installing the driver w/o having 1394 generate a 
    // devobj for us
    //
    if (NULL == pNodeExt->PortDeviceObject)
    {
        return STATUS_UNSUCCESSFUL;
    }
    ...
    ntStatus = WdfIoTargetOpen (deviceExtension->PortDeviceIoTarget, &openParams);
    if (!NT_SUCCESS (ntStatus)) 
    {
        TRACE (TL_ERROR, ("WdfIoTargetCreate failed 0x%x\n", ntStatus));
        return ntStatus;
