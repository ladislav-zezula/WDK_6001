...
--*/
{
    ...
    if (SUCCEEDED(hr))
    {
        m_ReadWriteQueue->StartFxQueue();
        m_ControlQueue->StartFxQueue();        
    }
...
--*/
{
    ...
    //
    // Ideally we would have used IWDFIoQueue::StopSynchronously but
    // StopSynchronously doesn't work correctly in UMDF 1.5, so we
    // call the async method Stop and set an event in the state change
    // callback
    //

    m_ReadWriteQueue->StopFxQueueSynchronously();
    m_ControlQueue->StopFxQueueSynchronously();
