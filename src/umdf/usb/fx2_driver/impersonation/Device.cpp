...

{
    ...
    //
    // Make the request cancellable using our driver's 
    // cancel callback object.
    //

    cancel = CCancelCallback::CreateAndInitialize();
    ...
    FxRequest->MarkCancelable(cancel->QueryIRequestCallbackCancel());
    cancel->Release();
            ...
            if (result)
            {
                ...
                if (bytesRead > 0)
                {
                    if(EncodeSegmentValue(c, &segment) == true)
                    {
                        ...
                        cancel->WaitForEvent(PlayInfo->Delay);
                    }
                }
                else
                {
    ...
    //
    // Request is no longer cancelable.  If we can't then the request has 
    // been cancelled or is being cancelled.  In the later case we wait on 
    // the event in the cancel object before completing the request so
    // that we know that the framework won't touch the request again.
    //

    if (FxRequest->UnmarkCancelable() == HRESULT_FROM_WIN32(ERROR_OPERATION_ABORTED))
    {
        cancel->WaitForEvent(INFINITE);
    }
