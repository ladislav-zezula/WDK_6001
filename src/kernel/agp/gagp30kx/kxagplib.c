...
--*/
{
    ...
    //
    // KeNumberProcessors removed from Vista SP1/Vista Server build environment headers to force hot plug CPU awareness
    // KeNumberProcessors replaced with KeQueryActiveProcessorCount()
    // Driver does NOT support hot plug CPU but is compatible with Vista SP1/Vista Server build environment
    //
    // Using PAGED_CODE() as KeQueryActiveProcessorCount() must be called at IRQL <= APC_LEVEL
    //

    PAGED_CODE();
    ActiveProcessorCount = KeQueryActiveProcessorCount(NULL);

    //
    // Search for K8/9 CPU(s) on the primary bus
    //
    KxAgpCtx.KxCount = 0;
    SearchDepth = MIN(MAX_KX_AGP_DEVICE, ActiveProcessorCount);
    for (Index = 0;
